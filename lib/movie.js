...
exports.prepareVideo = function (data) {
  ...
  if(config.skip.intro === false) {
    tasks.push(function (callback) {
      ...
      var chunks = utils.chunk(data.drivers, 15);
      var index = 0;
      ...
      var lastname = '[intro]';
      var imagestart = 9;
      var length = parseInt(9/chunks.length);
      ...
      chunks.forEach(function(chunk) {
        ...
        // add overlay
        filters.push(lastname+"["+(index+2)+":v] overlay=0:0:enable='between(t,"+imagestart+","+(imagestart+length)+")' [e"+index+"]");
  ...
  // audio for intro
  if(config.skip.music === false && fs.existsSync(path.resolve(__dirname, audio.music)) === true){
    tasks.push(function (callback) {
      ffmpeg()
  ...
  if(config.skip.end === false) {
    tasks.push(function (callback) {
      ...
      // calculate how many drivers we have
      var chunks = utils.chunk(data.drivers, 15);
      var index = 0;
      ...
      var lastname = '[faded]';
      var imagestart = 2;
      var length = parseInt(16/chunks.length);
  ...
  if(config.skip.music === false && fs.existsSync(path.resolve(__dirname, audio.music)) === true){
    tasks.push(function (callback) {
      ffmpeg()
          ...
          .on('start', function (commandLine) {
            console.log('🎥 - Race results audio');
            //console.log(commandLine);
          })
          .on('end', function () {
            callback();
  ...
  tasks.push(function (callback) {
    ...
    for (var i = 0; i < chunks.length; i++) {
      final.input(path.resolve(__dirname, '../tmp/race-' + i + '.mp4'));
    }
...
function sliceVideo(callback, data, previousEnd, video, id, items, duration) {
  ...
  data.forEach(function (chunk) {
    ...
    // structure data
    var cstart = chunk.start - start;
    var cend = ((chunk.end === 0) ? chunk.start : chunk.end ) - start;
    ...
    // add image input
    slicer.input(path.resolve(__dirname, chunk.filename));

    // add filter
    raceFilters.push(prevName + videoName + " overlay=0:0:enable='between(t," + cstart + "," + cend + ")' " + nextName);
   ...
   slicer
      ...
      .on('start', function (commandLine) {
        console.log('✂️ - Slicing & rendering racing video '+id);
        //console.log(commandLine);
      })
      .on('end', function () {
        callback();
