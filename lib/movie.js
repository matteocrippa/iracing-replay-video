...
exports.prepareVideo = function (data) {
  ...
  const items = parseInt(data.standings.length / 4);
  ...
  async.waterfall([
    ...
    function (callback) {
      generateRaceVideo(callback, data, video, [0, items]);
    },
    function (callback) {
      generateRaceVideo(callback, data, video, [items+1, (items*2)]);
    },
    function (callback) {
      generateRaceVideo(callback, data, video, [(items*2)+1, (items*3)]);
    },
    function (callback) {
      generateRaceVideo(callback, data, video, [(items*3)+1, data.standings.length]);
    },
    function (callback) {
      ffmpeg()
...
function generateRaceVideo(callback, data, video, bounds) {
  ...
  for (var i = bounds[0]; i < bounds[1]; i++) {
    ...
    const index = i - bounds[0];

    const videoName = "[" + (index + 1) + ":v]";
    const prevName = (index === 0) ? "[0:v]" : "[combo" + (index - 1) + "]";
    const nextName = "[combo" + index + "]";
