...
exports.generateLeaderBoards = function(session, data, camDrivers, configuration) {
  ...
  if(configuration.fastestLap.active) {
    boards = populateFastestLap(boards, configuration);
  }

  // populate fastest lap
  if(configuration.incidents.active) {
    boards = populateYellowFlag(boards);
  }
...
function populateYellowFlag(boards) {

  // loop race events
  boards.events.forEach(function(event){

    const ff = {
      start: parseFloat(event.StartTime._text),
      end: EndTime(event.StartTime._text),
...
function populateFastestLap(boards, configuration) {
