...
function populateYellowFlag(boards, events) {
  ...
  events.forEach(function (event) {
    ...
    const ff = {
      start: parseFloat(event.StartTime._text)+2, // need to add a fake delay or the flag will be shown earlier than expected
...
function populateFastestLap(boards, fastest, configuration) {
  ...
  fast.forEach(function (ff) {
    ...
    _.filter(boards, function (item) {
      ...
      const fl = _.find(item.order, function (it) {
