...
exports.generateLeaderBoards = function(session, data, camDrivers, fastestLaps) {
  var boards = [];
  var lastTime = 0.0;
  var lastFastest = 0.0;
  ...
  boards = populateFastestLap(boards, fastestLaps);
...
function populateFastestLap(boards, fastest) {

  fastest.forEach(function(current){
    const driver = current.Driver.CarNumber._text;
    const laptime = utils.convertTimeToRacing(current.Time._text);
    const when = parseFloat(current.StartTime._text);

    // TODO: optimize
    boards.forEach(function(board){
      if(board.start > when && board.hasFastest === false) {
        // set it has fastest lap
        board.hasFastest = true;

        // set the fastest lap owner
        const fl = _.find(board.order, function(item) { return item.driver.number === driver });
        fl.isFastestLap = true;
        fl.fastestLap = laptime;
      }
    });
